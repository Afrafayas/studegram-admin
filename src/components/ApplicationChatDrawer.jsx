    ...
    <>
      ...
      <aside className="fixed inset-y-0 right-0 max-w-md w-full bg-white shadow-2xl z-50 flex flex-col h-full transform transition-transform duration-300 animate-in slide-in-from-right">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 scrollbar-thin">
            ...
            messages.map((msg, index) => {
              ...
              return (
                ...
                >
                    ...
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[11px] font-semibold shadow-xs leading-relaxed space-y-2 ${
                      ...
                      {msg.attachments && msg.attachments.length > 0 && (
                        <div className="pt-2 border-t border-white/20 space-y-2">
                          <p className="text-[9px] font-extrabold uppercase tracking-wide opacity-80">Attachments ({msg.attachments.length})</p>
                          <div className="grid grid-cols-1 gap-1.5">
                            {msg.attachments.map((fileUrl, fIdx) => {
                              const filename = fileUrl.split('/').pop().replace(/mock-file-\d+-/, '');
                              if (isImage(fileUrl)) {
                                return (
                                  <a 
                                    key={fIdx} 
                                    ...
                                    target="_blank" 
                                    rel="noreferrer"
                                    className="block border border-white/25 rounded-lg overflow-hidden bg-black/10 hover:opacity-90 transition-opacity max-w-full"
                                  >
                                    <img 
                                      src={fileUrl} 
                                    ...
                                    />
                                    <div className="bg-black/40 px-2 py-1 text-[8px] truncate font-mono">{filename}</div>
                                  </a>
                                );
                              }
                              ...
                              return (
                                <a 
                                  ...
                                  target="_blank" 
                                  rel="noreferrer"
                                  className="flex items-center gap-2 p-2 bg-black/10 hover:bg-black/20 border border-white/20 rounded-lg text-white font-semibold hover:no-underline transition-colors max-w-full truncate"
                                >
                                  ...
                                  <div className="truncate flex-1 text-[10px] leading-tight font-mono">
                                    {filename}
                                  </div>
                                </a>
                              );
                  ...
                  {isFromMe && (
                    <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-white text-xs bg-slate-800 shrink-0 shadow-sm uppercase">
                      {senderInitials}
                    </div>
                  )}
                </div>
          ...
          {isSending && (
            <div className="flex items-start gap-2.5 max-w-[85%] ml-auto justify-end animate-pulse">
              <div className="space-y-1 max-w-[85%]">
